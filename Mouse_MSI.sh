...
BAIT=/project/GP1/u3710062/AI_SHARE/GATK/Outputs/20200507_MouseMSI/For_Picard/cl7_cMNR-panel_regions_cons.interval_list
TARGET=/project/GP1/u3710062/AI_SHARE/GATK/Outputs/20200507_MouseMSI/For_Picard/cl7_cMNR-panel_capture_targets.interval_list
...
while read -r Tumor Ctrl_1 Ctrl_2 Ctrl_3 Ctrl_4 Ctrl_5 Ctrl_6 Ctrl_7;
 ...
 for i in "${array[@]}"
	...
	echo '
		...
		BAIT='${BAIT}'
		TARGET='${TARGET}'
	' > ${wkdir}/run_${Tumor}_${i}_mutect2.sh
	### To add GATK preprocess and Mutect2 -L
	tail -n +37 ${SCRIPT_PATH}/run_mutect2.sh >> ${wkdir}/run_${Tumor}_${i}_mutect2.sh
        ### To add VEP
	tail -n +11 ${SCRIPT_PATH}/run_vep.sh >> ${wkdir}/run_${Tumor}_${i}_mutect2.sh
	chmod 755 ${wkdir}/run_${Tumor}_${i}_mutect2.sh
	qsub ${wkdir}/run_${Tumor}_${i}_mutect2.sh
	sleep 10s
	done
